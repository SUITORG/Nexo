...
function callDirectGoogle(messages) {
  return new Promise((resolve, reject) => {
    ...
    const opts = {
      ...
      path: `/v1/models/gemini-2.0-flash:generateContent?key=${FALLBACK_GEMINI_KEY}`,
      method: 'POST',
    ...
    const req = https.request(opts, (res) => {
      ...
      res.on('end', () => {
        ...
        try {
          ...
          circuitBreaker.recordSuccess('google/gemini-2.0-flash', Date.now() - start);
          resolve({ model: 'google/gemini-2.0-flash', content: text, latency: Date.now() - start });
        } catch (e) {
