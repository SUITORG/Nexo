                ...
                try {
                    ...
                    if (imageFiles.length > 0) {
                        ...
                        for (let i = 0; i < imageFiles.length; i++) {
                            ...
                            const cmd = `ffmpeg -y -loop 1 -t ${timePerSlide} -i "${imgPath}" -vf "scale=1080:1920:force_original_aspect_ratio=decrease,pad=1080:1920:(ow-iw)/2:(oh-ih)/2,fps=${fps}" -c:v libx264 -pix_fmt yuv420p -preset ultrafast "${segPath}"`;
                            execSync(cmd, { timeout: 60000, shell: true, stdio: 'pipe' });
                            segments.push(segPath);
                        }
                        ...
                        execSync(`ffmpeg -y -f concat -safe 0 -i "${concatFile}" -c copy "${outPath}"`, { timeout: 60000, shell: true, stdio: 'pipe' });

                        // Add audio if available
                        if (fs.existsSync(voiceFile)) {
                            const finalPath = path.join(tmpDir, 'vide_with_audio.mp4');
                            try {
                                execSync(`ffmpeg -y -i "${outPath}" -i "${voiceFile}" -c:v copy -c:a aac -shortest "${finalPath}"`, { timeout: 60000, shell: true, stdio: 'pipe' });
                                fs.unlinkSync(outPath);
