...
async function handleIncoming(payload, whatsappConfig, geminiKey) {
  ...
  const reservacionesLevel = parseInt(empresa.usa_reservaciones) || 0;
  if (reservacionesLevel < 1) {
    const msgText = 'Lo sentimos, el servicio de reservaciones no está disponible en este momento.';
    await sendMessage(msg.from, msgText, whatsappConfig.phoneId, whatsappConfig.token);
    return { ok: false, reason: 'reservations_disabled' };
  }
  ...
  const calendarId = reservacionesLevel >= 2 ? (empresa.id_calendario_google || null) : null;
